# Question:
# Across the United States, how have emissions from coal combustion-related 
# sources changed from 1999–2008?

library(dplyr)
library(ggplot2)

#Unzip source file
print("Extracting source files from zip file ...")
unzip("exdata%2Fdata%2FNEI_data.zip")

#Load data from source files
print("Loading source files into R ...")
NEI <- readRDS("summarySCC_PM25.rds")
SCC <- readRDS("Source_Classification_Code.rds")

#Merge NEI coal records with SCC
print("Merging NEI with SCC ...")
NEI_SCC <- merge(NEI, SCC, by.x = "SCC", by.y = "SCC")

#Create dataframe with only coal combustion records
print("Filtering data for coal combustion sources ...")

coal_data <-NEI_SCC %>% select(year, EI.Sector, Emissions) %>% 
                        filter(grepl("Coal", EI.Sector)) %>% 
                        group_by(year) %>% 
                        summarize(Total_Emissions = sum(Emissions))

#Plot ...
print("Plotting total emissions ...")
dev.set(2)
qplot(year, Total_Emissions, 
            data = coal_data, 
            geom = "line", 
            xlab = "Year", 
            ylab = "Total Emissions", 
            main = "Total Coal Combustion Emissions 1999-2008")



#Save plot as png
print("Saving PNG file ...")
dev.copy(png, "plot4.png")
dev.off()

print("Plot complete.")